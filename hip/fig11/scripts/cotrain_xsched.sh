...
docker run \
    ...
    -e XSCHED_AUTO_XQUEUE_THRESHOLD=32 \
    -e XSCHED_AUTO_XQUEUE_BATCH_SIZE=16 \
    -e XSCHED_AUTO_XQUEUE_PRIORITY=0 \
    rocm/pytorch:rocm5.6_ubuntu20.04_py3.8_pytorch_1.12.1 \
    bash -c "cd ${TESTCASE_ROOT_CONTAINER}; \
             python ./training/shufflenet.py -n 1; \
             LD_PRELOAD=${XSCHED_SHIM} python ./training/shufflenet.py \
             1> ${RESULT_DIR_CONTAINER}/cotrain_xsched_ojob_mi50.json" &
...
docker run \
    ...
    -e XSCHED_POLICY=GBL \
    -e XSCHED_AUTO_XQUEUE=ON \
    -e XSCHED_AUTO_XQUEUE_THRESHOLD=128 \
    ...
    bash -c "cd ${TESTCASE_ROOT_CONTAINER}; \
             python ./training/shufflenet.py -n 1; \
             LD_PRELOAD=${XSCHED_SHIM} python ./training/shufflenet.py \
             1> ${RESULT_DIR_CONTAINER}/cotrain_xsched_pjob_mi50.json" &

...
def main(num_epochs, save_path):
    ...
    # Training loop
    def train(epoch, total_start_time):
        model.train()
        running_loss = 0.0
        correct = 0
        total = 0
        ...
        for i, (inputs, labels) in enumerate(trainloader):
            ...
            print(f"{{\"time\": \"{current_time}\", "
                ...
                f"\"total thpt (iter/s)\": {(epoch * len(trainloader) + i + 1) * batch_size / (end - total_start_time):.3f}, "
    ...
    with torch.cuda.stream(stream):
        ...
        for epoch in range(num_epochs):
            ...
            train_loss, train_acc = train(epoch, total_start_time)
            # test_loss, test_acc = evaluate()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--num_epochs", "-n", type=int, default=100000)
    parser.add_argument("--save_path", "-o", type=str, default=None)
    args = parser.parse_args()
    main(args.num_epochs, args.save_path)

...
export XSCHED_AUTO_XQUEUE_TIMESLICE=800
...
LD_PRELOAD=${TESTCASE_ROOT}/output/lib/libshimascend.so ${TESTCASE_ROOT}/output/bin/up_fg \
...
sleep 2
export XSCHED_AUTO_XQUEUE_THRESHOLD=3
export XSCHED_AUTO_XQUEUE_BATCH_SIZE=2
export XSCHED_AUTO_XQUEUE_UTILIZATION=25
LD_PRELOAD=${TESTCASE_ROOT}/output/lib/libshimascend.so ${TESTCASE_ROOT}/output/bin/up_bg \
    ${ROOT}/assets/models/ascend \
    ${RESULT_DIR}/up_xsched_${dev_name}.thpt &
BG_PID=$!
echo "BG_PID: ${BG_PID}"

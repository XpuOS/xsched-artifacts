...
static void load_driver_libraries() {
  void *table = NULL;
  char driver_filename[] = "/usr/lib/x86_64-linux-gnu/libnvidia-ml.so.1";
  int i;
...
static void load_cuda_single_library(int idx) {
  void *table = NULL;
  char cuda_filename[] = "/usr/lib/x86_64-linux-gnu/libcuda.so";
...
int read_controller_configuration() {
  ...
  if (unlikely(rsize <= 0)) {
    LOGGER(4, "can't read integers from %s, error %s", config_path, strerror(errno));
    goto DONE;
  }
  buffer[rsize] = '\0';

  sscanf(buffer, "%d,%zu", &util_limit, &gpu_memory);
  ...
  g_vcuda_config.limit = util_limit;
  g_vcuda_config.utilization = util_limit;
  g_vcuda_config.gpu_memory = gpu_memory;

  g_vcuda_config.hard_limit = 1;

...
static const long long update_rate_limit(int *p_state, CUdevice device, double recv_rate, double max_rate, double *p_max_rate) {
  ...
  double delta_signed = delta;
  delta = delta > 0 ? delta : -delta;

  fprintf(stderr, "p_state: %s, recv_rate: %lf, max_rate: %lf, delta: %lf\n",
          *p_state == TGS_SLOW_START ? "TGS_SLOW_START" : "TGS_CONGESTION_AVOIDANCE",
          recv_rate, max_rate, delta_signed);
  ...
  {
  case TGS_SLOW_START:
    // if (delta <= threshold) { // fix: if current_rate >= (1 + threshold) * max_rate, should increase too
    if (delta_signed >= -threshold) {
  ...
  fprintf(stderr, "set rate limit to: %lld\n", rate_limit);
...
profile:
    for (int t = 1; t <= PREWARM_TIME + PROFILE_TIME || continue_flag; ++t) {
      ...
      if (max_delta >= -0.05 && max_delta <= 0.05) {
        ...
        // continue_flag = (abs(max_rate - max_window_rate) < 1e-5); // fix: if max_rate is stable, continue_flag should be 0

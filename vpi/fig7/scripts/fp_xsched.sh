...
LD_PRELOAD=${TESTCASE_ROOT}/output/lib/libshimvpi.so ${TESTCASE_ROOT}/output/bin/fp_bg \
    ${device} \
    ${media} \
    ${cmd_cnt} &
BG_PID=$!
...
LD_PRELOAD=${TESTCASE_ROOT}/output/lib/libshimvpi.so ${TESTCASE_ROOT}/output/bin/fp_fg \
    ${device} \
    ${media} \
    ${cmd_cnt} \
    ${fg_thpt} \
    ${RESULT_DIR}/fp_xsched_${device}.cdf &
FG_PID=$!
echo "FG_PID: ${FG_PID}"

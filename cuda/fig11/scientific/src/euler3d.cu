...
#define test_cnt 1000000 // never stop
#define warmup_cnt 10
// #ifndef block_length
...
template <typename T>
void copy(T* dst, T* src, int N, cudaStream_t stream = 0)
{
...
void compute_step_factor(int nelr, float* variables, float* areas, float* step_factors, cudaStream_t stream = 0)
{
...
void compute_flux(int nelr, int* elements_surrounding_elements, float* normals, float* variables, float* fluxes, cudaStream_t stream = 0)
{
...
void time_step(int j, int nelr, float* old_variables, float* variables, float* step_factors, float* fluxes, cudaStream_t stream = 0)
{
...
{
  fprintf(stderr, "WG size of kernel:initialize = %d, WG size of kernel:compute_step_factor = %d, WG size of kernel:compute_flux = %d, WG size of kernel:time_step = %d\n", BLOCK_SIZE_1, BLOCK_SIZE_2, BLOCK_SIZE_3, BLOCK_SIZE_4);

	if (argc < 2)
	{
		std::cerr << "specify data file name" << std::endl;
		return 0;
	}
	...
	for (int i = 0; i < warmup_cnt; i++) run(iterations);
	...
	for (int i = 0; i < test_cnt; i++) {
		...
		double total_us = std::chrono::duration_cast<std::chrono::microseconds>(end - total_start).count();
		double thpt = iterations / us * 1000000;
		double total_thpt = iterations * (i + 1) / total_us * 1000000;
		printf("{\"time\": \"%02d:%02d:%02d.%05ld\", \"test_id\": %d, \"iter_cnt\": %d, \"latency (ms)\": %.3f, \"throughput (iters/s)\": %.4f, \"total_throughput (iters/s)\": %.4f},\n",
			   now->tm_hour, now->tm_min, now->tm_sec, ms, i, iterations, us / 1000, thpt, total_thpt);
		fflush(stdout);
	}

	// std::cerr << "Saving solution..." << std::endl;
	// dump(variables, nel, nelr);
	// std::cerr << "Saved solution..." << std::endl;

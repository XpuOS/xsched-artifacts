...
int ivpu_jsm_hws_suspend_cmdq(struct ivpu_device *vdev, u32 ctx_id, u32 cmdq_id)
{
	struct vpu_jsm_msg req = { .type = VPU_JSM_MSG_HWS_SUSPEND_CMDQ };
	struct vpu_jsm_msg resp;
	int ret;

	req.payload.hws_suspend_cmdq.host_ssid = ctx_id;
	req.payload.hws_suspend_cmdq.reserved_0 = 0;
	req.payload.hws_suspend_cmdq.cmdq_id = cmdq_id;

	ret = ivpu_ipc_send_receive(vdev, &req, VPU_JSM_MSG_HWS_SUSPEND_CMDQ_DONE, &resp,
				    VPU_IPC_CHAN_ASYNC_CMD, vdev->timeout.jsm);
	if (ret)
		ivpu_warn_ratelimited(vdev, "Failed to suspend command queue: %d\n", ret);

	return ret;
}

int ivpu_jsm_hws_resume_cmdq(struct ivpu_device *vdev, u32 ctx_id, u32 cmdq_id)
{
	struct vpu_jsm_msg req = { .type = VPU_JSM_MSG_HWS_RESUME_CMDQ };
	struct vpu_jsm_msg resp;
	int ret;

	req.payload.hws_resume_cmdq.host_ssid = ctx_id;
	req.payload.hws_resume_cmdq.reserved_0 = 0;
	req.payload.hws_resume_cmdq.cmdq_id = cmdq_id;

	ret = ivpu_ipc_send_receive(vdev, &req, VPU_JSM_MSG_HWS_RESUME_CMDQ_RSP, &resp,
				    VPU_IPC_CHAN_ASYNC_CMD, vdev->timeout.jsm);
	if (ret)
		ivpu_warn_ratelimited(vdev, "Failed to resume command queue: %d\n", ret);
